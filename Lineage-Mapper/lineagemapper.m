 ...
 Module[{potentialparentstodaughter,
  ...
  daughtermappingOtherCells,mappeddaughters,trueElems},
    ...
    multimappings= Normal@Map[(
     Select[KeySelect[GroupBy[Join[Normal@#,PtoFmapping],First-> Last,Union],<|Thread[Keys[#]-> True]|>],
...
identifyFusions[currSeg_,prevSeg_,overlapMat_,trackvector_,OptionsPattern@cellTracker]:= With[{
  ...
  Module[{fmatrix, fvector, nrow, ncol, nullcandidates, areaprev= Values@ComponentMeasurements[prevSeg,"Area"], indices},
   ...
   Normal@GroupBy[
    Cases[GatherBy[indices, First],{p:Repeated[{_,_},{2,\[Infinity]}]}:> p], First->Last]
    (* we can also use Gather[indices, First@#2 \[Equal] First@# &] instead of GatherBy *)
  ]
]
