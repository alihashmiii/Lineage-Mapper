...
potentialDPoverlap,indices, multipledaughterspairs, parents, selcosts, getcosts,selpos,parentsKeys,selpairsAssociation,PtoFmapping,
FtoPmapping,multimappings,mothercircularity,combinedFrameHist, areaCurr,areaPrev, isTrueDivisionQ,picks,daughtermappingOtherCells,
mappeddaughters,trueElems},
...
Note: in some cases Fusion may be desirable, but i have not implemented it. *)
breakingFusion[currSeg_,prevSeg_,parentkeys_,OptionsPattern@cellTracker][rule_]:= Block[{maxlabel, newLabels,
  blobind,cluster,tempSeg,unassignedpos,t,$i=1,nf,replaceResidual,blobpos,masks, boundmasks,intersectionpos,
  symindices, keys, rules, newlabelblob, labelfill, keyscurr,conflicts,replaceRulesCurr, toreplace},
  ...
  blobpos = SparseArray[Unitize[newlabelblob - tempSeg]/. _Unitize -> 1,Automatic, 1]["NonzeroPositions"]; (* position of blob
  in current frame *) 
  ...
  unassignedpos = SparseArray[Unitize[newlabelblob - tempSeg]/._Unitize -> 1,Automatic,1]["NonzeroPositions"]; (* residual part
  of blob *)
  nf = Nearest@Flatten@MapThread[Thread[#1->#2]&,{Array[t,$i-1],cluster}];
  replaceResidual = Replace[Map[#->nf[#,1]&, unassignedpos], HoldPattern[p:{_,_}-> {x_}]:> p -> x,{1}]; (* assign nearest label to 
  residual position *)
...
(* the function operates on the cost matrix etc.. to create a mapping between the source and target cells if any. Any unassigned
cells @t are either dead or mitotic mother cells. Similarly any unassigned cells @t+1 either enter the FOV or are daughters.
Fused cells once broken maintain their original labels. Here we use a graph with edgeweights to find unique mappings *)
assignmentFunc[segCurr_,trackvector_,costMat_, splitcells_,mdpairs_,truePrevKeys_]:= Module[{segmentCurr= segCurr,
 previousCells,currentCells,currframelabels, currlabelsUntracked, daughters, mothers, realindices,artificialInds,newlabels,
...
daughterlabels,previnds,selfRules},
...
artificialInds = trackvector/.Rule -> List;
previnds = Part[truePrevKeys,trackvector[[All,1]] ];
realindices = Transpose[{previnds,Part[trackvector,All,2]}];
...
keeplabelsRules},
 ...
 If[Length@$framehistory <= frameCircCheck,
  ...
  need to be recomputed: CHANGE THIS STRING *)
  If[fusionsTindex != {},
   segmentCurr = Fold[breakingFusion[#,segmentPrev,truePrevkeys][#2]&,segmentCurr,fusionsTindex]
   ];
